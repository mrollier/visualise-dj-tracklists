...
import { COLOR_SCHEMES, makeNodeColor, MISSING_COLOR } from '../src/core/scales'

describe('makeNodeColor', () => {
  test('rating uses the ordinal ramp: 0 darkest, 5 lightest, null missing', () => {
    const color = makeNodeColor('rating', [0, 5], 'blue')
    expect(color(0)).toBe(COLOR_SCHEMES.blue[0])
    expect(color(5)).toBe(COLOR_SCHEMES.blue[5])
    expect(color(null)).toBe(MISSING_COLOR)
  })

  test('rating values are clamped and rounded into the six steps', () => {
    const color = makeNodeColor('rating', [0, 5], 'blue')
    expect(color(7)).toBe(COLOR_SCHEMES.blue[5])
    expect(color(2.6)).toBe(COLOR_SCHEMES.blue[3])
  })

  test('bpm interpolates continuously across the ramp endpoints', () => {
    const color = makeNodeColor('bpm', [100, 180], 'blue')
    expect(color(100)).toBe(COLOR_SCHEMES.blue[0])
    expect(color(180)).toBe(COLOR_SCHEMES.blue[5])
    const mid = color(140)
  ...
  test('every scheme has six validated steps', () => {
    for (const ramp of Object.values(COLOR_SCHEMES)) {
