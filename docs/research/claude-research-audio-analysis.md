...
- ~~**WS2 — Offline analyser**: `scripts/analyse-audio.py` + pure helpers in `scripts/analysis-lib.mjs` (location decoding, resumable merge, confidence gate) with vitest coverage, mirroring genre-pack-lib precedent. Model execution verified by real-library run + fixture-WAV smoke test.~~ **BUILT 2026-08-26 as v34** — see [designs/design-v34-offline-analyser.md](../designs/design-v34-offline-analyser.md). Two departures: `analysis-lib.mjs` was **dropped** (the genre-pack precedent had a *JavaScript* producer; here it is Python, so a JS helper module would hold nothing real — `tests/analysis-contract.test.ts` verifies through the app's own `mergeAnalysis` instead), and the Pearson-r report is gone with the calibration premise above. **Full user value lands here** is right, but the value is **energy**, not BPM/key: see WS3.
- ~~**WS3 — In-app gap-fill analyser**: AnalysisPanel.svelte (folder grant / drag-drop, progress per-track postMessage, cancel = flag + worker.terminate; partial results commit per-track), analyser.ts orchestration, worker.ts (essentia.js WASM BPM+key). First IndexedDB use (persisted directory handle — tiny, isolated).~~ **REJECTED 2026-08-26 on measurement, not preference.** The "~55 gap tracks" this workstream exists to fill do not exist: the 22 missing BPM and 33 missing key **overlap**, and 30 of the 33 are the Rekordbox sampler content §1 separately warns about, plus 2 Pioneer demo tracks. **The real gap is 1 track**, and a hand-written one-entry sidecar closes it with no code. Two further corrections for anyone tempted to revive it: the folder grant, IndexedDB handle persistence, directory walk and non-Chromium picker fallback all shipped in **v28** (`src/lib/audio/`), so "first IndexedDB use" is wrong; and Chromium's ffmpeg has `CONFIG_AIFF_DEMUXER 0`, which puts 772 of 2080 tracks (37%) out of reach of `decodeAudioData` without an extra decoder.
